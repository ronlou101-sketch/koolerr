...
export function getDeliveryDepartmentHealth(): DeliveryDepartmentHealth {
  ...
  // overall aligns with readyForDelivery: the department can run when the primary is up.
  // A missing fallback is not a blocking condition — it degrades resilience, not capability.
  const overall: DeliveryProviderReadiness = primaryConfigured ? 'ready' : 'not-configured'
