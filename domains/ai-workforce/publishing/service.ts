import { modelGateway } from '@/shared/model-gateway'
import type { IModelGateway } from '@/shared/model-gateway'
import { trustEngine } from '@/shared/trust'
import { logger } from '@/shared/lib/logger'
import { ok, err } from '@/shared/types'
import type {
  DigitalEmployeeId,
  ModelProvider,
  OrganizationId,
  Result,
  TrustRule,
} from '@/shared/types'
import { WORKFORCE_REGISTRY } from '../employees'
import { buildPublishingPrompt, parsePublishingPackages, PUBLISHING_SYSTEM_CONTEXT } from './prompt'
import type {
  PublishingError,
  PublishingErrorCode,
  PublishingJob,
  PublishingPackage,
  PublishingRequest,
} from './types'

const MAX_RETRIES = 3
const PUBLISHING_ACTION = 'prepare_publishing_packages'

// Text providers for publishing package generation.
// buildProviderOrder() starts from the employee's declared primaryProvider ('openai'),
// which IS in this list — so the order is ['openai', 'anthropic'] as intended.
const PUBLISHING_PROVIDERS: ModelProvider[] = ['openai', 'anthropic']

/** Public interface consumed by the orchestrator and downstream systems. */
export interface IPublishingDepartmentService {
  preparePackages(request: PublishingRequest): Promise<Result<PublishingJob, PublishingError>>
  getJob(jobId: string): PublishingJob | undefined
  listJobs(): PublishingJob[]
}

export class PublishingDepartmentService implements IPublishingDepartmentService {
  private readonly jobs = new Map<string, PublishingJob>()
  private readonly gateway: IModelGateway

  constructor(gateway?: IModelGateway) {
    this.gateway = gateway ?? modelGateway
  }

  async preparePackages(
    request: PublishingRequest
  ): Promise<Result<PublishingJob, PublishingError>> {
    const employeeId = request.preferredEmployee ?? 'delivery-manager'
    const employee =
      WORKFORCE_REGISTRY.find((e) => e.id === employeeId) ??
      WORKFORCE_REGISTRY.find((e) => e.id === 'delivery-manager')!

    this.ensureTrustRule(employee.id as DigitalEmployeeId, request.organizationId)

    const jobId = crypto.randomUUID()
    const job: PublishingJob = {
      id: jobId,
      status: 'running',
      videoProductionBrief: request.videoProductionBrief,
      packages: [],
      attempts: 0,
      employeeId: employee.id,
      providerId: PUBLISHING_PROVIDERS[0],
      createdAt: new Date(),
      updatedAt: new Date(),
    }
    this.jobs.set(jobId, { ...job })

    logger.info('[PUBLISHING_DEPT] Job started', { jobId, employee: employee.id })

    const providerOrder = this.buildProviderOrder(
      employee.primaryProvider,
      employee.fallbackProvider
    )

    return this.executeWithRetry(job, request, providerOrder)
  }

  /**
   * Resolves provider order for publishing package generation.
   *
   * The delivery-manager has primaryProvider: 'openai' which IS in PUBLISHING_PROVIDERS,
   * so buildProviderOrder('openai', null) → ['openai', 'anthropic'].
   * The implicit fallback loop appends any remaining providers not yet in the order.
   */
  private buildProviderOrder(primary: string, fallback: string | null): ModelProvider[] {
    const order: ModelProvider[] = []

    if (PUBLISHING_PROVIDERS.includes(primary as ModelProvider)) {
      order.push(primary as ModelProvider)
    }

    if (
      fallback &&
      PUBLISHING_PROVIDERS.includes(fallback as ModelProvider) &&
      fallback !== primary
    ) {
      order.push(fallback as ModelProvider)
    }

    for (const provider of PUBLISHING_PROVIDERS) {
      if (!order.includes(provider)) order.push(provider)
    }

    if (order.length === 0) order.push('openai')

    return order
  }

  private async executeWithRetry(
    job: PublishingJob,
    request: PublishingRequest,
    providerOrder: ModelProvider[]
  ): Promise<Result<PublishingJob, PublishingError>> {
    let lastError = ''

    for (const providerId of providerOrder) {
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        job.attempts++
        job.updatedAt = new Date()

        if (attempt > 1) {
          job.status = 'retrying'
          this.jobs.set(job.id, { ...job })
          logger.info('[PUBLISHING_DEPT] Retrying', {
            jobId: job.id,
            provider: providerId,
            attempt,
          })
        }

        try {
          const packages = await this.invokePackageGeneration(request, job, providerId)

          job.status = 'completed'
          job.packages = packages
          job.providerId = providerId
          job.updatedAt = new Date()
          this.jobs.set(job.id, { ...job })

          logger.info('[PUBLISHING_DEPT] Job completed', {
            jobId: job.id,
            provider: providerId,
            packages: packages.length,
            attempts: job.attempts,
          })

          return ok({ ...job })
        } catch (error) {
          lastError = String(error)
          logger.warn('[PUBLISHING_DEPT] Attempt failed', {
            jobId: job.id,
            provider: providerId,
            attempt,
            error: lastError,
          })

          if (this.isNonRetriable(lastError)) {
            break
          }
        }
      }
    }

    const errorCode = this.classifyError(lastError)
    job.status = 'failed'
    job.error = lastError
    job.updatedAt = new Date()
    this.jobs.set(job.id, { ...job })

    logger.error('[PUBLISHING_DEPT] Job failed — all providers exhausted', {
      jobId: job.id,
      attempts: job.attempts,
      error: lastError,
    })

    return err({
      code: errorCode,
      message: `Publishing job failed after ${job.attempts} attempt(s): ${lastError}`,
      retriable: errorCode !== 'MAX_RETRIES_EXCEEDED' && errorCode !== 'TRUST_ENGINE_DENIED',
    })
  }

  private async invokePackageGeneration(
    request: PublishingRequest,
    job: PublishingJob,
    providerId: ModelProvider
  ): Promise<PublishingPackage[]> {
    const prompt = buildPublishingPrompt(request.videoProductionBrief)

    const response = await this.gateway.invoke({
      tenantId: request.tenantId,
      organizationId: request.organizationId,
      workforceId: request.workforceId,
      digitalEmployeeId: job.employeeId as DigitalEmployeeId,
      engagementRunId: request.engagementRunId,
      action: PUBLISHING_ACTION,
      prompt,
      systemContext: PUBLISHING_SYSTEM_CONTEXT,
      provider: providerId,
      maxTokens: 8192,
    })

    return parsePublishingPackages(response.content, request.videoProductionBrief)
  }

  private ensureTrustRule(employeeId: DigitalEmployeeId, organizationId: OrganizationId): void {
    const rule: TrustRule = {
      id: `publishing-${employeeId}-${PUBLISHING_ACTION}`,
      organizationId,
      digitalEmployeeId: employeeId,
      action: PUBLISHING_ACTION,
      requiresApproval: false,
      autonomyLevel: 'autonomous',
    }
    trustEngine.registerRule(rule)
  }

  private isNonRetriable(error: string): boolean {
    return (
      error.includes('OPENAI_API_KEY') ||
      error.includes('No provider available') ||
      error.includes('trust_engine_denied') ||
      error.includes('Trust Engine denied')
    )
  }

  private classifyError(error: string): PublishingErrorCode {
    if (error.includes('OPENAI_API_KEY') || error.includes('No provider available')) {
      return 'PROVIDER_NOT_CONFIGURED'
    }
    if (error.includes('trust_engine_denied') || error.includes('Trust Engine denied')) {
      return 'TRUST_ENGINE_DENIED'
    }
    if (error.includes('did not complete within') || error.includes('timeout')) {
      return 'TIMEOUT'
    }
    if (
      error.includes('non-JSON') ||
      error.includes('missing or invalid field') ||
      error.includes('missing or empty') ||
      error.includes('missing array field') ||
      error.includes('Missing or invalid') ||
      error.includes('Missing or empty')
    ) {
      return 'INVALID_RESPONSE'
    }
    return 'MAX_RETRIES_EXCEEDED'
  }

  getJob(jobId: string): PublishingJob | undefined {
    const job = this.jobs.get(jobId)
    return job ? { ...job } : undefined
  }

  listJobs(): PublishingJob[] {
    return Array.from(this.jobs.values()).map((j) => ({ ...j }))
  }
}

/** Singleton publishing department service. Injected with the platform Model Gateway. */
export const publishingDepartment: IPublishingDepartmentService = new PublishingDepartmentService()
