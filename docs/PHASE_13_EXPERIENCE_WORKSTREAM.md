...
| **C** | Pipeline → "New campaign" modal                | 🔄 **Implemented — awaiting Preview approval**      | Campaigns (`/runs`) is now the single entry point for creating a campaign: a "New campaign" button opens a modal hosting the shared `CampaignCreator` (extracted from the old `/pipeline` page — same `POST /api/pipeline/run` engine). Pipeline removed from everyday nav (`MORE_NAV`); `/pipeline` route preserved as a thin wrapper. Runs→Campaigns copy refreshed. Surface-only; no engine/API/schema change; Home untouched. tsc + 849 tests + build clean. Awaiting founder Preview approval before Production.                  |
...
**Slices A and B are complete and live in production (deployed 2026-07-29, koolerr.vercel.app).
Slice C is implemented (Pipeline → "New campaign" modal) and awaiting founder Preview approval
before Production.** Next candidate is **Slice D — Creative → "Generate image" modal** (not started;
