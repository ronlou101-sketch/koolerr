...
| 2     | Campaign Rendering          | 🔄 **In progress** — decomposed into slices **CR-1…CR-n** (see `docs/CAMPAIGN_RENDERING_IMPLEMENTATION_PLAN.md`). **CR-1 — Gateway Brand-Identity Contract ✅ complete** (`780373b`, pushed; founder-approved). **CR-2 — Render Path Consolidation ✅ complete** (`867b200`, pushed; founder-approved). **CR-3 — Brand Ambassador injection ✅ complete** (`f643dde`, pushed; founder-approved): the single render path resolves the org's ambassador and injects its provider-agnostic identity into the gateway (ADR-025 §1/§7). **CR-4 — Spokesperson-video entitlement enforcement ✅ implemented, awaiting founder review** (not yet committed/pushed): video renders are gated on `checkEntitlement('spokesperson_video')` before dispatch (over-limit → `ENTITLEMENT_EXCEEDED` → HTTP 402, non-spending) and metered via `recordUsageEvent` on success (ADR-025 §4, Option A); image renders not gated. Adds `spokesperson_video` to `UsageEventType` + additive migration 023 (CHECK widening); reuses existing billing APIs only; no gateway/adapter change. tsc + 886 tests + build clean. **CR-5+ not started** (require founder approval). |
...
**CR-4 — Spokesperson-video entitlement enforcement ✅ implemented, awaiting founder review** (not yet
committed/pushed): video renders gated on `checkEntitlement('spokesperson_video')` before dispatch
...
**Next: CR-5** (not started; requires founder approval before it begins).
