...
and pushed** (`4cc1f78`; ADR-025 §2/§6). **CR-6b — durable `render_jobs` model ✅ implemented,
awaiting founder review** (not yet committed/pushed): additive migration 024 (`render_jobs` table +
atomic `claim_render_jobs()` SQL function) + a new inert `render-jobs` module (idempotent enqueue,
atomic bounded claim, retry-aware fail); strictly additive + inert (ADR-025 §5).
**Next: CR-6c** — driver + pipeline enqueue; not started; requires founder approval and its own plan.
