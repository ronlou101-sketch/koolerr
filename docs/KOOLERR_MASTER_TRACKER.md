...
| 2     | Campaign Rendering          | 🔄 **In progress** — decomposed into slices **CR-1…CR-n** (see `docs/CAMPAIGN_RENDERING_IMPLEMENTATION_PLAN.md`). **CR-1 — Gateway Brand-Identity Contract ✅ complete** (`780373b`, pushed; founder-approved). **CR-2 — Render Path Consolidation ✅ complete** (`867b200`, pushed; founder-approved). **CR-3 — Brand Ambassador injection ✅ complete** (`f643dde`, pushed; founder-approved): the single render path resolves the org's ambassador and injects its provider-agnostic identity into the gateway (ADR-025 §1/§7). **CR-4 — Spokesperson-video entitlement enforcement ✅ complete** (`2f69fd9`, pushed; **founder-accepted**): video renders are gated on `checkEntitlement('spokesperson_video')` before dispatch (over-limit → `ENTITLEMENT_EXCEEDED` → HTTP 402, non-spending) and metered via `recordUsageEvent` on success (ADR-025 §4, Option A); image renders not gated. Adds `spokesperson_video` to `UsageEventType` + additive migration 023 (CHECK widening); reuses existing billing APIs only; no gateway/adapter change. tsc + 886 tests + build clean. **CR-5 — Campaign report renderer fix ✅ implemented, awaiting founder review** (not yet committed/pushed): the deliverable detail page rendered a `type:'report'` deliverable blank (GenericContent read `body`/`contentBrief`/`draft`, absent from a `DeliveryPackage`); CR-5 adds a pure, unit-tested `toReportView` normalizer + a `ReportContent` renderer showing the real package fields (summary, what's included, platform packages, publishing instructions, schedule, quality review). Display-only; no backend/API/schema/migration change. tsc + 891 tests + build clean. **CR-6 — pipeline render-wiring (Workstream B-orchestration + §5 async job model) remains and will need its own decomposition; not started** (requires founder approval). |
...
**CR-5 — Campaign report renderer fix ✅ implemented, awaiting founder review** (not yet
committed/pushed): the deliverable detail page rendered `type:'report'` deliverables blank; CR-5 adds a
pure, tested `toReportView` normalizer + a `ReportContent` renderer for the real `DeliveryPackage`
fields (display-only; ADR-025 §6/§8).
**Next: CR-6** — pipeline render-wiring (Workstream B-orchestration + §5 async job model); not started;
requires founder approval and its own decomposition.
