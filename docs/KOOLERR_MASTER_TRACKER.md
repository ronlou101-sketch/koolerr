...
CR-6c (driver + enqueue). CR-6a — pipeline emits spokesperson `video_script` deliverable ✅
implemented, awaiting founder review** (not yet committed/pushed): a canonical `writeScript()` in the
Video Production dept + one bounded, non-fatal `video_script` deliverable from the pipeline (strictly
additive; ADR-025 §2/§6).
**Next: CR-6b** — durable `render_jobs` model; not started; requires founder approval and its own plan.
