...
- [x] Password reset regression fixed (B-006): email link no longer fails with `otp_expired` — scanner-proof `/confirm` page deployed; `{{ .TokenHash }}` flow active
- [x] Password reset verified end-to-end in production: request link → receive email → click link → set new password → sign in
...
**2026-06-28** (B-006 production verification confirmed — B-006 closed)
