...
| 2     | Campaign Rendering          | 🔄 **In progress** — decomposed into slices **CR-1…CR-n** (see `docs/CAMPAIGN_RENDERING_IMPLEMENTATION_PLAN.md`). **CR-1 — Gateway Brand-Identity Contract ✅ complete** (`780373b`, pushed; founder-approved). **CR-2 — Render Path Consolidation ✅ complete** (`867b200`, pushed; founder-approved). **CR-3 — Brand Ambassador injection ✅ implemented, awaiting founder review** (committed locally, not pushed): the single render path now resolves the org's ambassador (`brandAmbassadorService.resolveBrandAmbassador`) and maps it to the gateway's provider-agnostic `BrandIdentity` (avatar/voice ← `providerRefs.heygen`, `referenceImageUrls`, `seed`), so every render is branded; falls back to the platform default + logs when no ambassador (ADR-025 §1/§7). Single-site change (only `render.ts`); Business Brain stays provider-agnostic; tsc + 880 tests + build clean. **CR-4+ not started** (require founder approval). No entitlement, orchestration, gateway/adapter change, or migration yet. |
...
**CR-3 — Brand Ambassador injection ✅ implemented, awaiting founder review** (committed locally, not
pushed): the single render path resolves the org's ambassador and injects its provider-agnostic
identity into the gateway so every render is branded, with platform-default fallback (ADR-025 §1/§7).
**Next: CR-4** (not started; requires founder approval before it begins).
